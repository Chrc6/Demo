 ...
 */
/* package */ final class CustomSonic2 {
   ...
   */
  public CustomSonic2(int inputSampleRateHz, int numChannels, float speed, float pitch,
    ...
    inputBuffer = new int[maxRequired * numChannels];
    outputBufferSize = maxRequired;
    outputBuffer = new int[maxRequired * numChannels];
    ...
    loudnessInsurer.setBytesPerSample(2);
    loudnessInsurer.initInstance(inputSampleRateHz, numChannels);
    Log.i("sonic==="," inputSampleRateHz="+inputSampleRateHz+" numChannels="+numChannels+" speed="+speed
              +" pitch="+pitch+" outputSampleRateHz="+outputSampleRateHz);
  }
   ...
   */
  public void queueInput(IntBuffer buffer) {
   ...
   */
  public void getOutput(IntBuffer buffer) {
...
//    byte[] bytes = new byte[outputBuffer.length * 2];
//    ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN).asShortBuffer().put(outputBuffer);
//    info.byteBuffer = bytes;
//    info.bufferSize = bytes.length;
//    BufferInfo out = new BufferInfo();
//    loudnessInsurer.onPcm(info, out, 0);
//
//    short[] shorts = new short[out.bufferSize/2];
//    ByteBuffer.wrap(out.byteBuffer).order(ByteOrder.LITTLE_ENDIAN).asShortBuffer().get(shorts);
//    outputBuffer = shorts;
//    outputBufferSize = shorts.length;
  }
...
//    int fixedPointVolume = (int) (volume * 4096.0f);
    int fixedPointVolume = (int) (1.0f * 4096.0f);
...
//      int value = (samples[xSample] * fixedPointVolume) >> 12;
      int value = (samples[xSample] * fixedPointVolume) >> 12;
      Log.i("sonic==="," value="+value+" xSample="+xSample+" min="+min+" max="+max);
//      if (value > 32767) {
//        value = 32767;
//      } else if (value < -32767) {
//        value = -32767;
