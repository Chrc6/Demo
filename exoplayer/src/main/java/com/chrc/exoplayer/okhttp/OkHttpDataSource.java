...
public class OkHttpDataSource implements HttpDataSource {
  ...
  public long open(DataSpec dataSpec) throws HttpDataSourceException {
    ...
    Log.i("exoplayer===okhttp", "open lenth="+dataSpec.length+" time="+time);
    System.out.println("exoplayer===okhttp open lenth="+dataSpec.length+" bytesRead="+bytesRead);
    ...
    Log.i("exoplayer===okhttp", "open request before time="+curTime
    ...
    int size = 0;
    try {
      size = responseByteStream.available();
    } catch (IOException e) {
      e.printStackTrace();
    }
    Log.i("exoplayer===okhttp", "open request after time="+System.currentTimeMillis()+" interval="+(System.currentTimeMillis() - curTime)
  ...
  public void close() throws HttpDataSourceException {
    Log.i("exoplayer===okhttp", "close opened="+opened);
    System.out.println("exoplayer===okhttp close opened="+opened);
    if (opened) {
      opened = false;
      if (listener != null) {
  ...
  private int readInternal(byte[] buffer, int offset, int readLength) throws IOException {
    if (System.currentTimeMillis() - returnTime > 10 * 1000) {
      Log.i("rendererl===", "okhttpdatasource overtime="+(System.currentTimeMillis() - returnTime));
      //缓冲下一块数据
      startReadTime = 0;
      bytesCount = 0;
    }
    ...
    Log.i("rendererl===", "okhttpdatasource readInternal start readLength="+readLength+" bytesRemaining="+(bytesToRead - bytesRead));
    if (read == -1) {
      if (bytesToRead != C.LENGTH_UNSET) {
    ...
    bytesRead += read;
    bytesCount += read;
    if (bytesCount > 200 * 1024) {
      Log.i("rendererl===", "okhttpdatasource inteval="+(System.currentTimeMillis() - startReadTime)+" size="+bytesCount);
      startReadTime = 0;
      bytesCount = 0;
    }
    if (listener != null) {
